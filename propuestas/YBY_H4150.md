...
`nota` Apariencia dinámica de color y fuente en app para clientes por nutricionista

### Inicio / Mensajes
_Y el apartado de mensajes_

`nota` Quién manda los mensajes y desde dónde, ¿se puede hacer algo con ellos aparte de verlos? ¿Los mensajes son los posts?
...
`nota` Empesa nutricionista 1M nutricionistas y uno o varios son administradores

`nota` ¿quén mantiene las etiquetas de clientes y desde dónde? ¿según se escriben y son por nutricionista?
...
`nota` Actividad reciente ¿de dónde (qué datos) se obtiene (acciones a mostrar como rellenó formulario, escribió en el chat, etc.)?
...
+ Vista de calendario de cliente `nota:` ¿una columna por día como en app de muestra?
+ UC:
    + Crear un evento (competición, `nota:` ¿tipos?)
...
`nota` ¿Desde dónde se generan los cuestionarios? ¿Cómo se envían al cliente? ¿Qué tipos hay? ¿Periódicos?
...
`nota` ¿Desde dónde se generan los cuestionarios? ¿Cómo se piden al cliente? ¿Los envía el cliente _motu proprio_? ¿Qué tipos hay? ¿Periódicos?

`nota`+ BD datos de evolución de cliente ¿necesaria / cuál es su estructura?
...
+ Cálculo de tasa metabólica basal `nota` ¿fórmula según perfil del cliente? + superavit / déficit calórico

+ Listado de métodos de cálculo o fórmulas (ver imagen)

`nota` _se usarán datos del cuestionario_ > mejor de la ficha del cliente. 

`nota` _objetivo marcado en el cuestionario_ > mejor de la ficha del cliente. 

`nota` Definir datos de entrada, fórmulas y estética de la calculadora. 

`nota` Los resultados de estos cálculos ¿son únicos por cliente? ¿se almacenan de forma periódica?  

+ UI Vista de cálculo energético por cliente

+ BD: Resultados de cálculos energético por cliente `nota` ¿necesario / estructura?
...
+ UC: `nota`Generación automática con IA ¿datos del prompt? ¿formato de salida? ¿datos de consulta (alimentos)?
